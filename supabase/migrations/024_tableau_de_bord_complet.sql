/*
  MAbeautyplus V2 — Migration 024 : le tableau de bord reprend ce que
  l'ancien savait faire

  S'ajoutent aux chiffres existants :
    · le chiffre d'affaires par centre et par thérapeute ;
    · le tableau croisé centre × mois sur six mois ;
    · les vingt dernières ventes ;
    · l'évolution en pourcentage contre la période précédente, de même
      durée — c'est ce qui rend un chiffre lisible : 3 104 € ne dit rien,
      « 3 104 €, en baisse de 12 % » dit quelque chose ;
    · un filtre par thérapeute, en plus du filtre par centre.

  Les répartitions portent sur le SIGNÉ, comme dans l'ancien tableau de
  bord : c'est la vente qu'on attribue à un centre et à une thérapeute, pas
  l'encaissement, qui tombe des mois plus tard.
*/

DROP FUNCTION IF EXISTS tableau_de_bord(text, date, date);
...
BEGIN
  ...
  -- La période précédente, de même durée, collée juste avant.
  v_duree   := (p_au - p_du) + 1;
  v_au_prec := p_du - 1;
  ...
  bilans_periode AS (
    SELECT b.id, b.profil_dominant, b.terrain_dominant
    FROM bilans b
  ...
  -- Six mois de croisé centre × mois
  mois6 AS (
    SELECT generate_series(
  ...
  croise AS (
    SELECT
      ce.id AS centre_id,
      ce.nom AS centre,
      to_char(mois6.m, 'YYYY-MM') AS mois,
      (SELECT COALESCE(SUM(p.montant_total), 0)
         FROM programmes p
        WHERE p.statut <> 'abandonne'
  ...
  SELECT jsonb_build_object(
    ...
    'dernieres_ventes', (SELECT COALESCE(jsonb_agg(x ORDER BY x->>'date' DESC), '[]'::jsonb)
      FROM (
        ...
        WHERE p.statut <> 'abandonne'
          ...
          AND (p_centre IS NULL OR p.centre_id = p_centre)
          AND (p_therapeute IS NULL OR p.therapeute_id = p_therapeute)
        ORDER BY p.date_validation DESC, p.cree_le DESC
        LIMIT 20) s),
    ...
    'attendu', (SELECT jsonb_build_object(
      'reste', reste, 'retard_montant', retard_montant, 'retard_nb', retard_nb,
      'semaine_montant', semaine_montant, 'semaine_nb', semaine_nb) FROM attendu),
