/*
  MAbeautyplus V2 — Migration 028 : une fiche archivée ne doit plus rien

  Archiver, c'est sortir une fiche du suivi sans rien perdre. Mais ses
  échéances impayées continuaient de compter dans « Reste à encaisser » et
  « En retard » : on lisait un impayé de 391 € sans pouvoir retrouver la
  cliente, puisqu'elle n'apparaît plus dans aucune liste. Un chiffre qu'on
  ne peut pas rattacher à quelqu'un ne sert à rien — on ne relance pas un
  montant.

  Ce qui reste dû par une fiche archivée n'est donc plus compté.

  Le chiffre d'affaires, lui, ne bouge pas : ce qui a été signé et encaissé
  l'a bien été, même si la fiche a été archivée depuis. On n'efface pas une
  vente en rangeant un dossier.

  Reste une question à trancher côté application : faut-il prévenir au
  moment d'archiver une fiche qui doit encore de l'argent ?
*/
