/*
  MAbeautyplus V2 — Migration 054 : les frais Alma ne sont pas du chiffre
  d'affaires du centre

  CE QU'ON CORRIGE.

  Chez Alma, les échéances portent ce que règle la cliente, frais compris.
  C'est juste : c'est bien ce qu'elle voit sur son relevé, et le contrat de
  crédit l'annonce ainsi. Mais ces frais ne rémunèrent aucune prestation du
  centre — ils sont le coût du crédit et ils vont à l'organisme.

  Le tableau de bord les comptait dans l'encaissé, et attribuait donc au
  centre un chiffre d'affaires qu'il n'a jamais reçu : sur une cure à
  1 269 €, 1 357 € encaissés.

  CE QUE ÇA DÉPLACE DANS VOS CHIFFRES : l'encaissé BAISSE, du montant des
  frais Alma de la période. Ce n'est pas de l'argent perdu, c'est de
  l'argent qui n'a jamais été le vôtre. La correction va donc dans le sens
  inverse de la 053, qui l'avait fait monter.

  CE QUI NE CHANGE PAS : le signé, qui compte `montant_total` — déjà hors
  frais. Ni « Montant Cure » dans Airtable, qui porte ce que règle la
  cliente, frais compris, parce que c'est ce que les relances annoncent.

  LE RESTE DE LA FONCTION EST REPRIS MOT POUR MOT DE LA 028. Seules trois
  sommes changent, et elles passent toutes par `part_centre`.
*/

/*
  La part d'un règlement qui revient au centre.

  Au prorata : tout réglé, le centre a sa cure entière ; rien réglé, il n'a
  rien. Sans frais — les chèques du centre — elle ne change rien.
*/
CREATE OR REPLACE FUNCTION part_centre(
  montant      numeric,
  montant_cure numeric,
  frais        numeric
)
RETURNS numeric
LANGUAGE sql
...
AS $fn$
  SELECT CASE
    WHEN COALESCE(frais, 0) <= 0
      OR COALESCE(montant_cure, 0) + COALESCE(frais, 0) <= 0
    THEN montant
    ELSE ROUND(montant * montant_cure / (montant_cure + frais), 2)
  END;
$fn$;

REVOKE ALL ON FUNCTION part_centre(numeric, numeric, numeric) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION part_centre(numeric, numeric, numeric) TO authenticated;
...
/*
  Contrôle. `ecart` est ce que les frais Alma pesaient à tort dans
  l'encaissé de toute l'histoire. Il doit correspondre à la somme des frais
  des cures Alma dont les échéances sont réglées.
*/
SELECT
  ROUND(SUM(e.montant), 2)                                                        AS avec_frais,
  ROUND(SUM(part_centre(e.montant, p.montant_total, p.frais_financement)), 2)     AS part_du_centre,
  ROUND(SUM(e.montant - part_centre(e.montant, p.montant_total, p.frais_financement)), 2) AS ecart
FROM echeances e
JOIN programmes p ON p.id = e.programme_id
WHERE e.statut = 'paye'
