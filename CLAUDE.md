...
| Connexion | **Un compte par thérapeute**, pas par centre. Cloisonnement par centre en RLS. Un compte `direction` voit les 5 centres. Les adresses suivent `prenom.centre@mabeautyplus.fr`, sans accent ni majuscule : deux Alexandra travaillent dans deux centres, et le centre dans l'adresse évite l'homonymie pour de bon. Les comptes se créent dans Supabase (Authentication → Add user, **Auto Confirm** coché), puis `supabase/rattacher_les_comptes.sql` les relie et dit qui manque. |
