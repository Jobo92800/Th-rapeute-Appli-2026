...
| Parcours audio | L'application « Mon Parcours » (`Jobo92800/Applipodcast`, `applipodcast.netlify.app`) reste **séparée** : son projet Supabase, son site Netlify, ses fonctions. La V2 se contente de créer le compte de la cliente au moment de la signature du contrat, avec le parcours A/B/C **choisi par la thérapeute**. **Il n'y a aucun lien personnel à récupérer** : l'application appelle Supabase Auth qui envoie l'invitation elle-même. Le lien d'invitation est à usage unique et expire en 24 h — ne jamais chercher à le stocker. Dans Airtable, le champ formule `Lien parcours audio` renvoie simplement l'adresse du site quand `Accès audio` est rempli. |
