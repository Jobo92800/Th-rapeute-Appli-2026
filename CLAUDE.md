...
| Règlement | **Au centre par chèques**, de 1 à 4 fois sans frais : les échéances suivent les séances, et le guide et la tenue tombent sur la première — la cliente repart avec. **Ou Alma par carte**, en 2, 3, 4, 10 ou 12 fois, avec des frais à la charge de la cliente (0,87 % à 7,5 %) portés par `frais_financement`. Les anciennes valeurs `4x_maison` et `10x_alma` restent acceptées pour les cures déjà signées. |
