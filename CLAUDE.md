...
| Règlement | Les mêmes conditions partout — premier bilan comme cure suivante, un seul calcul couvert par le banc d'essai. **Au centre par chèques**, de 1 à 4 fois sans frais — **et 5 fois à partir de 20 luxo** (15 septembre 2026, migration 062, `SEANCES_LUXO_POUR_CINQ_CHEQUES`) : c'est la cure la plus longue et la plus chère, la seule où étaler sur cinq mois se justifie ; vingt I-Shape durent aussi cinq mois et restent à quatre. Même règle sous « Changer le nombre d'échéances » — : les échéances suivent les séances, et le guide et la tenue tombent sur la première — la cliente repart avec. Le prix d'une séance étant rond, ces chèques le sont aussi, et ils le restent quand un bilan ou un acompte a déjà été versé (voir « Bilan réglé en ligne »). Un montant s'écrit d'ailleurs au plus juste à l'écran, `formaterEurosJuste` : les centimes seulement quand il y en a — « 413 € » se lit d'un coup, « 413,00 € » demande de vérifier les deux zéros, et les mensualités Alma gardent les leurs. Rien n'est jamais arrondi à l'affichage. **Ou Alma par carte**, en 2, 3, 4, 10 ou 12 fois, avec des frais à la charge de la cliente, portés par `frais_financement`. **Les
...
`npm test` — **616 contrôles, à garder verts.** Ils couvrent ce qui décide
...
Migrations passées jusqu'à **058** incluse (la 054, la 057, la 059, la **060** à la **069** sont écrites, à passer ; la 060 et la 062 demandent aussi de redéployer `synchro-airtable`), fonction Edge `gerer-les-comptes` déployée et vérifiée de bout en bout, `synchro-airtable` redéployée,
